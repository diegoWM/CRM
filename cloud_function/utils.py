...
def process_accounts_data(data):
    ...
    try:
        ...
        for col in date_columns:
            if col in df.columns:
                try:
                    ...
                    # Ensure the column still exists but with NaT values for failed conversions
                    df[col] = pd.to_datetime(df[col], errors='coerce')
        ...
        for col, dtype in numeric_columns.items():
            if col in df.columns:
                try:
                    ...
                    logger.info(f"Successfully converted column {col} to {dtype.__name__}")
                except Exception as e:
                    logger.warning(f"Error converting {col} to {dtype.__name__}: {str(e)}")
                    # Ensure the column still exists but with NaN values for failed conversions
        ...
        logger.error(f"Error processing accounts data: {str(e)}")
        raise
...
def validate_accounts_data(df):
    ...
    try:
        ...
        logger.info("Data validation complete: No critical issues found")
        return True
    except Exception as e:
...
def save_to_gcs(data, prefix):
    ...
    try:
        ...
        try:
            ...
            error_msg = f"GCS bucket {BUCKET_NAME} not found or not accessible: {str(bucket_error)}"
            logger.error(error_msg)
            raise ValueError(error_msg)
        ...
        try:
            ...
            error_msg = f"Failed to serialize data to JSON: {str(json_error)}"
            logger.error(error_msg)
            raise ValueError(error_msg)
        ...
        logger.error(f"Failed to save data to GCS: {str(e)}")
        raise
